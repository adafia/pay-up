    ...
    async upload(req, res) {
        try {
            if (!req.files) {
                ...
                if(extension != 'xlsx' && extension != 'xsl') {
                    return res.status(400).send('Please upload an excel sheet')
                }
                ...
                // if(!item.CustomerID){
                //     return res.status(400).send('All repayment upload records should have a CustomerID')
                // }

                // const customer = await Customer.findOne({ CustomerID: item.CustomerID })
                // if (!customer) {
                //     return res.status(404).send(`Customer with ID: ${item.CustomerID} does not exist`)
                // }

                // if(item.SeasonID) {
                //     const season = await Season.findOne({ SeasonID: item.SeasonID })
                //     if (!season) {
                //         return res.status(404).send(`SeasonID: ${item.SeasonID} does not exist`)
                //     }
                // }
